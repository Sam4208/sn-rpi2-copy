...
def figure (x,y,kind,date,length,name):
        ...
        if kind == 25:
            n = length/24
            ax.xaxis.set_major_locator(plt.MaxNLocator(8*n))
        else:
            pass
        ...
        plt.suptitle('Plottng date is : ' + date + ' plot for : '+ str(length) + ' hours')
        plt.legend()
        ax.set_ylim(10,40)
...
file_read = input('Which sensor temperature would you like to browse?[FG0/FG1/BP/sensor] :')
