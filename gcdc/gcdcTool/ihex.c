					...
					int *code)
{
	...
	*num = 0;
	if (theline[0] != ':')
	{
		printf("parse error: <%s>, line does not start with a ':'\n",theline);
		return(-1);
	}
	if (strlen(theline) < 11)
	{
		printf("parse error: <%s>, line too short\n",theline);
		return(-2);
	}
	ptr = theline+1;
	if (!sscanf(ptr, "%02x", &len))
	{
		printf("parse error: <%s>, unable to parse length\n",theline);
		return(-3);
	}
	ptr += 2;
	if ( strlen(theline) < (11 + (len * 2)) )
	{
		printf("parse error: <%s>, insuffient characters for len:%d\n",theline,len);
		return(-4);
	}
	if (!sscanf(ptr, "%04x", addr))
	{
		printf("parse error: <%s>, unable to parse address\n",theline);
		return(-5);
	}
	ptr += 4;
	...
	{
		printf("parse error: <%s>, unable to parse code\n",theline);
		return(-6);
	}
	ptr += 2;
	...
	while(*num != len) {
		...
		if (*num >= 256)
		{
			printf("parse error: <%s>, too many symbols\n",theline);
			return(-8);
		}
	}
	if (!sscanf(ptr, "%02x", &cksum))
	{
		printf("parse error: <%s>, unable to parse checksum\n",theline);
		return(-9);
	}
	if ( ((sum & 255) + (cksum & 255)) & 255 )
	{
		printf("parse error: <%s>, checksum error %02x != %02x\n",theline,sum,cksum);
...
/** Load an intel hex file into the supplied buffer
  ...
  */
int ihex_load_file( char *filename, uint8_t *memory, uint32_t *start, uint32_t *end , uint8_t fill, uint32_t mem_size )
{
	char line[1000];
	FILE *fin;
	uint32_t addr;
	...
	while (!feof(fin) && !ferror(fin)) {
		...
		if(!parse_hex_line(line, bytes, &addr, &n, &status))
		{
			if (status == 0) 
			{  /* data */
			        if(addr+offset >= 0xF600)
			        {
			                if(addr+offset < 0xF800) 
        			                printf("WARNING mem write in protected area 0x%04x line <%s>\n",addr+offset, line);
			                continue;
			        }
				for(i=0; i<=(n-1); i++) 
				{
				        ...
				        {
				                printf("ERROR exceeding storage allocation 0x%0x > 0x%0x\n",addr+offset, mem_size);
				        }
				        if(memory[addr+offset] != fill)
				        {
				                printf("WARNING mem overwrite at 0x%04x line <%s>\n",addr+offset, line);
				        }
			...
			if (status == 1) {  /* end of file */
				fclose(fin);
				if(verbose_flag) printf("   Loaded %d bytes between:", total);
				if(verbose_flag) printf(" %04X to %04X\n", minaddr, maxaddr);
				*start = minaddr;
				*end = maxaddr;
				return(0);
			}
	...
	printf("ERROR end of file reached without finding end file delimeter\n");
	return(-4);
}
